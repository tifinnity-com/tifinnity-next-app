  ...
  return (
    <div className="container max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        <div className="mb-8">
            ...
            <p className="text-muted-foreground">In-depth analysis of your mess performance.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <AnalyticsStatCard title="Total Revenue" value="₹27,060" change="+12.5%" icon={DollarSign} />
            <AnalyticsStatCard title="Total Subscriptions" value="190" change="+8.2%" icon={ShoppingCart} />
            <AnalyticsStatCard title="Total Orders" value="345" change="-3.1%" icon={Users} isDecrease />
        </div>

        <Card>
            <CardHeader>
                <CardTitle>Monthly Performance</CardTitle>
                <CardDescription>Revenue, subscriptions, and orders over the last 6 months.</CardDescription>
            </CardHeader>
            <CardContent className="h-96">
                <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={monthlyData}>
                        ...
                        <Line type="monotone" dataKey="subscriptions" stroke="#82ca9d" name="Subscriptions" strokeWidth={2} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Popularity of different subscription plans.</CardDescription>
                </CardHeader>
...
function AnalyticsStatCard({ title, value, change, icon: Icon, isDecrease = false }: { title: string, value: string, change: string, icon: React.ElementType, isDecrease?: boolean }) {
  return (
    <Card>
      ...
      <CardContent>
        ...
        <p className={`text-xs flex items-center ${isDecrease ? 'text-red-500' : 'text-green-500'}`}>
          {isDecrease ? <TrendingDown className="h-4 w-4 mr-1" /> : <TrendingUp className="h-4 w-4 mr-1" />}
